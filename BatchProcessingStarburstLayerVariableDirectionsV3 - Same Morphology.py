...
def run(trial_name, retina_parameters, diffusion_parameters, decay_parameters, conductance_parameters, stimulus_parameters):
...
#    decay_combinations = generateCombinations(decay_parameters, 0, [], [])    
    decay_combinations = generateCombinations(decay_parameters, 0, [], [])[0]
#    conductance_combinations = generateCombinations(conductance_parameters, 0, [], [])
    conductance_combinations = generateCombinations(conductance_parameters, 0, [], [])[0]
    ...
    print "Conductance Combinations", len(conductance_combinations)
    ...
    default_conductance = conductance_combinations[0] 
    ...
    for retina_combination in retina_combinations:
        ...
        retina_combination = retina_combination[0:18] + [default_decay, list(default_diffusion)] + retina_combination[18:23] + [default_conductance] + retina_combination[23:]
        ...
        for diffusion_combination in diffusion_combinations:
            ...
            for decay_combination in decay_combinations:
                ...
                for conductance_combination in conductance_combinations:
                    retina.on_starburst_layer.changeConductance(conductance_combination)        
                    print retina.on_starburst_layer.conductance_factor  
                    print retina.on_starburst_layer.neurons[0].conductance_factor
...
trial_name = "Bar_Diffusion_Speed_Same_Morphology_5ms_Timestep"
...
retina_parameters = parameters[0:18] + parameters[20:23] + parameters[24:24]
...
conductance_parameters = parameters[23]
if isinstance(conductance_parameters, float): conductance_parameters = [conductance_parameters]
stimulus_parameters = parameters[24:]  

# Start the trial
run(trial_path, retina_parameters, diffusion_parameters, decay_parameters, conductance_parameters, stimulus_parameters)