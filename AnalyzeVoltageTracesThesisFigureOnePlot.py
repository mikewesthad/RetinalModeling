...
def analyze(trial_path, retina_paths, number_stimulus_variations, parameter, selected_variations, number_bars=12):
    ...
    for retina_number in range(number_retinas):
        ...
        fig1_rows, fig1_cols, fig1_index = 1, 3, 1
        fig1 = createFigure(20.0*number_trials, fig1_rows, fig1_cols)   
        ...
        ax.table(cellText=cell_values, rowLabels=row_names,
                 bbox=(0.6, 0.45, 1.0/5.0, 1.0/2.0), colWidths=[0.2])
        
        for ((value, stimulus_number), fig1_index) in zip(selected_variations, [3]):
            ...
            for (compartment_index, compartment_name) in zip([2], ["Distal"]):
                ...
                for (heading_index, label, ls, c) in zip(range(4), labels, linestyles, colors):
                    ...
                    ax.text(0.05, 0.1, textstr, transform=ax.transAxes, fontsize=18,
...
#                ax.set_title(compartment_name)
        ...
        fig1.legend(handles, labels, bbox_to_anchor=(0.515, 1.0/4.0), loc='center', fontsize=20)
