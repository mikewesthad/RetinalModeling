...
def analyzeMorphology(trial_path, retina_paths, number_stimulus_variations, number_bars, x_axis_values, x_axis_label, x_axis_ticks_labels=None, figtitle=None, parameter=None, parameter2=None):
    ...
    fig1_rows, fig1_cols, fig1_index = number_retinas+1, number_retinas, 1
    fig1 = createFigure(18.0, fig1_rows, fig1_cols)
    grid_size = (fig1_rows, fig1_cols)
    
    plt.suptitle(figtitle, y=0.9275, fontsize=18)    
    
    indicies = range(fig1_cols)
    titles = x_axis_ticks_labels
    cols = range(fig1_cols)
    for (i, title, col) in zip(indicies, titles, cols):
        morphology_path   = os.path.join(os.getcwd(), trial_path, str(i)+" Morphology.jpg")
        morphology_image  = mpimg.imread(morphology_path)
        morphology_image  = morphology_image[100:900, 100:900]
        ax = plt.subplot2grid(grid_size, (0, col), colspan=1, rowspan=1)  
        ax.imshow(morphology_image)
        ax.axis('off')                    
        ax.set_title(title, size=15) 
        if i == 0:
            plt.text(0, 1.0, "A", transform=ax.transAxes,
                     fontsize=plot_label_size, verticalalignment="top", bbox=plot_label_props)
    
    first_retina_path   = os.path.join(os.getcwd(), trial_path, "0", "Vector Average Without Morphology.jpg")
    first_retina_image  = mpimg.imread(first_retina_path)
    first_retina_image  = first_retina_image[100:900, 100:900]
    ax = plt.subplot2grid(grid_size, (1, 0), colspan=number_retinas/2, rowspan=number_retinas/2) 
    ax.imshow(first_retina_image)
    ax.axis('off')   
    ax.set_title("{0} {1} Average Vector".format(titles[0], parameter), size=18)                  
    plt.text(plot_label_x, plot_label_y, "B", transform=ax.transAxes, 
             fontsize=plot_label_size, verticalalignment="top", bbox=plot_label_props)
             
    last_retina_path   = os.path.join(os.getcwd(), trial_path, str(number_retinas-1), "Vector Average Without Morphology.jpg")
    last_retina_image  = mpimg.imread(last_retina_path)
    last_retina_image  = last_retina_image[100:900, 100:900]
    ax = plt.subplot2grid(grid_size, (1, number_retinas/2), colspan=number_retinas/2, rowspan=number_retinas/2)
    ax.imshow(last_retina_image)
    ax.axis('off')                     
    ax.set_title("{0} {1} Average Vector".format(titles[-1], parameter2), size=18)
    ...
    ax =plt.subplot2grid(grid_size, (number_retinas/2+1, 0), colspan=number_retinas/2, rowspan=number_retinas/2) 
    ax.plot(x, y1, ls='-', color=y1_color)
    ax.fill_between(x, y1_min, y1_max, color=y1_err_color)   
    ax.set_title("Proximal Compartments", size=18)
    ax.set_xlabel(x_axis_label, size=14)
    ax.set_ylabel("AVM", color='b', size=14)
    ax2 = ax.twinx()
    ax2.plot(x, y2, ls='-', color=y2_color)
    ax2.fill_between(x, y2_min, y2_max, color=y2_err_color)
    ax2.set_ylabel("Centrifugality Index", color='r', size=14)
    ...
    y2_max  = np.array(y2) + np.array(y2_err)
    y2_min  = np.array(y2) - np.array(y2_err)
    ax = plt.subplot2grid(grid_size, (number_retinas/2+1, number_retinas/2), colspan=number_retinas/2, rowspan=number_retinas/2) 
    ax.plot(x, y1, ls='-', color=y1_color)
    ax.fill_between(x, y1_min, y1_max, color=y1_err_color) 
    ax.set_title("Distal Compartments", size=18)
    ax.set_xlabel(x_axis_label, size=14)
    ax.set_ylabel("AVM", color='b', size=14)
    ax2 = ax.twinx()
    ax2.plot(x, y2, ls='-', color=y2_color)
...
#    fig1.tight_layout(pad=2.0, w_pad=1.0, h_pad=1.0)
...
number_bars = 12
x_axis_ticks = [1, 2, 3, 4, 5, 6]
x_axis_ticks_labels = [1, 2, 3, 4, 5, 6]
x_axis_label = "Number Starting Dendrites"
number_stimulus_variations = 1
title = "Varying Number of Starting Dendrites"
parameter = "Starting Dendrite"
parameter2 = "Starting Dendrites"
...
analyzeMorphology(trial_path, retina_paths, number_stimulus_variations, number_bars, x_axis_ticks, x_axis_label, x_axis_ticks_labels, title, parameter, parameter2)